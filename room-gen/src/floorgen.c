...
void gen_floor() {
    ...
    /*for (UINT8 i = 0; i < 1; i++) {
        base_floor[i].is_small = TRUE;
        for (UINT8 j = 0; j < 4; j++) {
            base_floor[i].doors[j].open = TRUE;
            if (built[i].neighbours_ptr[j].is_null) {
                base_floor[i].doors[j].keys[0] = TRUE;
                base_floor[i].doors[j].keys[1] = TRUE;
                base_floor[i].doors[j].keys[2] = TRUE;
            } else {
                base_floor[i].doors[j].keys[0] = FALSE;
                base_floor[i].doors[j].keys[1] = FALSE;
                base_floor[i].doors[j].keys[2] = FALSE;
                base_floor[i].doors[j].ptr = built[i].neighbours_ptr[j].ptr;
            }
        }
        gen_room(&base_floor[i]);
    }*/
